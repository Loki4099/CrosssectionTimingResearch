...
| SEC基本面PIT源层 | **已冻结并认证** | 753/753研究期CIK完成；2,386,818条filing、1,679,666条注册事实、358,303条规范年度事实 | 保持源版本不变 |
| 市场与基本面因子面板 | **已冻结并审计** | 162个月末、17个登记因子、1,380,944行；14个因子通过数据门 | 制定首轮Top5/10/20/50单因子实验 |
...
- [紧凑因子数据库认证证据](../../results/published/cross_sectional_data/xs-market-sec-bundle-20260820-v1/)
...
在不改写本源层的前提下，派生因子数据库已经单独完成。17个登记因子中，11个市场因子以及`XS032_GROSS_PROFIT_AT`、`XS041_ASSET_GROWTH`通过数据门；严格Sloan应计覆盖不足，机械采用预登记的`XS056_CFO_ACCRUALS_PT`覆盖替代。`XS026_VALUE_BM`与`XS030_NET_PAYOUT_YIELD`因缺少经审计的发行人全股类历史市值而阻断。该资格只使用覆盖与可得性，不使用收益。复杂注释、分析师预期、期权和媒体数据不属于这一版本。
