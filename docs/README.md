...
| 主线 | 定位 | 当前状态 | 一站式入口 |
|---|---|---|---|
| 系统性 long-only 横截面 Alpha | 研究在历史时点S&P 500中持有哪些股票 | **当前主线**；市场与SEC源数据层已认证，因子库和首轮单因子计划待建 | [横截面Alpha主页](./research_tracks/cross_sectional_alpha.md) |
...
3. 完善因子定义、计算统一因子面板并做覆盖率QA；
4. 制定并执行论文定义的首轮单因子实验；
5. 根据单因子、相关性和集中度结果设计信号聚合；
6. 最后再比较模型、组合构建和冻结P00协同。

知识登记和源数据认证都不等于因子就绪或实验授权，也没有已经存在的“Round 11”结果。
