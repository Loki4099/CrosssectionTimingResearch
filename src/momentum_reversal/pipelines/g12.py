"""G12 continuous unlevered scaling driven by causal naked-book RV126."""
...
from .g11 import (
    _G11CoreBatch,
    _attach_regime_audit as _attach_continuous_regime_audit,
...
_FROZEN_DESIGN_SHA256 = (
    "5c8b66da4287b3229eb51cd6afb2fc0485df3515056cceb1b1e85e893dc0dcb9"
)
...
_FROZEN_GROUP_CONFIG_SHA256 = (
    "d1aed4c75d69b650e132d575864667153ca82baad0aef59f58159d8168d992fa"
)
...
class G12RunConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.context.group_id != "G12":
            raise ValueError("G12 runner requires the registered G12 spec")
        if isinstance(self.workers, bool) or not isinstance(self.workers, int):
            raise ValueError("G12 workers must be an integer")
        if self.workers <= 0 or self.workers > 8:
            raise ValueError("G12 workers must be between 1 and 8")
...
def continuous_book_allocation(
    regime: pd.DataFrame,
    ...
    *,
    ...
    """Return ``min(1, 0.15 / current naked-book RV126)``.
    ...
    if "book_realized_volatility" not in regime:
        raise ValueError("regime is missing book_realized_volatility")
    ...
    if not np.isclose(annual_target_volatility, 0.15, rtol=0.0, atol=1e-15):
        raise ValueError("G12 requires annual_target_volatility=0.15")
    if not np.isclose(maximum_scale, 1.0, rtol=0.0, atol=1e-15):
        raise ValueError("G12 requires maximum_scale=1.0")

    rv = pd.to_numeric(regime["book_realized_volatility"], errors="coerce")
    observed = rv.notna()
    invalid = observed & (~np.isfinite(rv) | rv.le(0.0))
    if invalid.any():
        raise DataQualityError(
            "G12 naked-book RV126 must be finite and positive when available: "
    ...
    if invalid_signal.any():
        raise DataQualityError(
            "G12 allocation is unavailable on signal dates: "
...
def run_g12(config: G12RunConfig) -> G12RunResult:
    """Run and freeze the 36-path G12 continuous naked-book RV126 grid."""
    ...
    if not summary["valid_scenario"].eq(True).all():
        raise DataQualityError("G12 completion gate found invalid scenarios")
    ...
    bundle: BundleWriteResult = write_experiment_bundle(
        ...
        resolved_config_toml=_render_g12_resolved_config_toml(
            config=config,
            data=data,
            reference_g00=reference_g00,
        ),
...
def _run_g12_core_batch_worker(
    context: ExperimentRunContext,
    ...
    try:
        ...
        raise ValueError(f"unknown G12 worker strategy: {error.args[0]}") from error
    return _run_g12_core_batch(context=context, data=data, strategies=strategies)


def _run_g12_core_batch(
    *,
    context: ExperimentRunContext,
    ...
    for strategy in strategies:
        ...
        _attach_continuous_regime_audit(strategy_summary, rebalances, audit_regime)
        rebalances.rename(
...
def _build_strategy_regimes(
    *,
    ...
    if len(expected_sessions) != 3_018:
        raise DataQualityError(
            f"G12 naked-book calendar changed: {len(expected_sessions)}"
        )
    long_only_engine = BaselineBacktester(
        data.prices,
        data.membership,
    ...
    for strategy in strategies:
        ...
        if len(extra_sessions):
            raise DataQualityError(
                f"G12 naked book has non-authoritative sessions for "
        ...
        regime["target_risk_allocation"] = continuous_book_allocation(regime)
        regime["target_scaled_source_volatility"] = (
            regime["book_realized_volatility"]
            * regime["target_risk_allocation"]
        )
        cap = pd.Series(pd.NA, index=regime.index, dtype="boolean")
        available = regime["book_realized_volatility"].notna()
        cap.loc[available] = regime.loc[
            available, "book_realized_volatility"
        ].le(0.15)
        regime["cap_is_binding"] = cap
        allocation = continuous_book_allocation(
...
def _validate_g00_path_identity(
    ...
    """Apply the audited G11 scalar gate after a lossless G12 ID alias."""
    ...
    for frame in (candidate_rebalances, candidate_holdings):
        ...
        if not identifiers.str.startswith("G12__").all():
            raise DataQualityError("G12 artifact contains a non-G12 strategy identity")
        frame["strategy_id"] = identifiers.str.replace(
            r"^G12__", "G11__", regex=True
        )
...
def _validate_artifact_counts(
    ...
    if len(data.evaluation_sessions) != 2_134 or len(nav) != 614_592:
        raise RuntimeError(
            "G12 requires 2134 sessions and 614592 NAV rows, got "
    ...
    if len(counts) != 288 or not counts.eq(2_134).all():
        raise RuntimeError("G12 every scenario must contain exactly 2134 NAV rows")
    required_pnl = [
        "pnl_total",
        "pnl_long_risk",
        "pnl_short_risk",
    ...
    if missing:
        raise DataQualityError(f"G12 NAV attribution lacks columns: {sorted(missing)}")
    numeric = nav[required_pnl].apply(pd.to_numeric, errors="coerce")
    if numeric.isna().any().any() or not np.isfinite(numeric).all().all():
        raise DataQualityError("G12 NAV attribution contains non-finite values")
    if numeric["pnl_unexplained_bridge"].abs().max() > 1e-10:
        raise DataQualityError("G12 unexplained daily P&L bridge exceeds tolerance")
    if numeric["pnl_closure_error"].abs().max() > 1e-12:
        raise DataQualityError("G12 daily P&L closure exceeds tolerance")
    unsupported = (
        numeric["pnl_action_execution_bridge"].abs().gt(1e-10)
        & numeric["pnl_bridge_has_frozen_evidence"].ne(1.0)
    )
    if unsupported.any():
        raise DataQualityError("G12 material action bridge lacks frozen evidence")
...
def _validate_daily_naked_regime(
    ...
    if missing:
        raise DataQualityError(
            f"G12 daily naked regime lacks columns: {sorted(missing)}"
        )
    if len(daily) != 108_648:
        raise RuntimeError(
            f"G12 requires 108648 persisted daily risk rows, got {len(daily)}"
        )
    ...
    if daily["date"].min() != _AUDIT_START or daily["date"].max() != data.evaluation_end:
        raise DataQualityError("G12 daily naked regime escaped the frozen interval")
    if daily.duplicated(["strategy_id", "date"]).any() or daily["strategy_id"].nunique() != 36:
        raise DataQualityError("G12 daily naked regime identities are invalid")
    counts = daily.groupby("strategy_id", observed=True).size()
    if not counts.eq(3_018).all():
        raise DataQualityError("G12 each path must persist exactly 3018 risk rows")
    ...
    for strategy_id, frame in daily.groupby("strategy_id", observed=True, sort=False):
        ...
        if not dates.equals(authoritative):
            raise DataQualityError(
                f"G12 daily dates differ from the calendar: {strategy_id}"
            )
        returns = pd.Series(
            pd.to_numeric(frame["book_return"], errors="coerce").to_numpy(dtype=float),
            index=dates,
        )
        expected = strict_lagged_book_quartiles(
            returns, realized_vol_window=126, history_sessions=756
        )
        expected["target_risk_allocation"] = continuous_book_allocation(expected)
        expected["target_scaled_source_volatility"] = (
            expected["book_realized_volatility"]
            * expected["target_risk_allocation"]
        )
        cap = pd.Series(pd.NA, index=dates, dtype="boolean")
        available = expected["book_realized_volatility"].notna()
        ...
        for column in (
            ...
            if not np.allclose(actual, wanted, rtol=0.0, atol=0.0, equal_nan=True):
                raise DataQualityError(
                    f"G12 stored daily {column} differs from direct calculation: "
                    f"{strategy_id}"
                )
        ...
        if not np.allclose(
            ...
            raise DataQualityError(
                "G12 stored daily volatility_quartile differs from direct "
    ...
    if rebalances[formal_required].isna().any().any():
        raise DataQualityError("G12 formal signal risk state is incomplete")
    formal_a = pd.to_numeric(rebalances["target_risk_allocation"], errors="coerce")
    formal_rv = pd.to_numeric(rebalances["book_realized_volatility"], errors="coerce")
    if (
        not np.isfinite(formal_a).all()
        or not np.isfinite(formal_rv).all()
        or formal_a.le(0.0).any()
        or formal_a.gt(1.0).any()
        or not np.allclose(
            formal_a.to_numpy(dtype=float),
            np.minimum(1.0, 0.15 / formal_rv.to_numpy(dtype=float)),
            rtol=0.0,
            atol=1e-15,
        )
    ):
        raise DataQualityError("G12 formal allocation violates the frozen rule")
...
def _attach_g00_comparisons(
    ...
    for index, row in summary.iterrows():
        ...
        parent = strategy_id.replace("G12__", "G00__", 1)
        if parent == strategy_id:
            raise DataQualityError(f"invalid G12 strategy identity: {strategy_id}")
        key = (parent, float(row["cost_bps"]), float(row["borrow_fee_annual"]))
        if key not in lookup.index:
            raise DataQualityError(f"G12 lacks matching G00 scenario: {key}")
    ...
    if len(comparison) != 1_440:
        raise RuntimeError(
            f"G12 must record 1440 G00 comparison rows, got {len(comparison)}"
        )
    return comparison
...
def _frozen_parameters(context: ExperimentRunContext) -> dict[str, object]:
    ...
    if not isinstance(raw, dict) or not isinstance(dates, dict):
        raise ValueError("G12 parameters or program dates are missing")
    ...
    for key, value in expected.items():
        ...
        if not matches:
            raise ValueError(f"G12 frozen parameter changed: {key}={actual!r}")
    history_start = str(dates.get("strategy_forecast_history_start", ""))
    if history_start != "2014-06-30":
        raise ValueError(f"G12 frozen history start changed: {history_start!r}")
    return {
...
def _validate_frozen_inputs(
    ...
    if context.dataset_version != _FROZEN_DATASET_VERSION:
        raise DataQualityError("G12 requires the frozen v3 dataset version")
    if data.dataset_manifest_sha256 != _FROZEN_DATASET_MANIFEST_SHA256:
        raise DataQualityError("G12 frozen dataset manifest hash changed")
    ...
    if (
        ...
        raise DataQualityError("G12 frozen dataset record hash changed")
    design = (
        context.project_root
        / "docs"
        / "20_experiments"
        / "G12_book_hist_continuous_scale"
        / "design.md"
    )
    if not design.is_file() or sha256_file(design) != _FROZEN_DESIGN_SHA256:
        raise DataQualityError("G12 preregistered design hash changed")
    ...
    if not program.is_file() or sha256_file(program) != _FROZEN_PROGRAM_SHA256:
        raise DataQualityError("G12 frozen experiment program hash changed")
    if sha256_file(context.group.path) != _FROZEN_GROUP_CONFIG_SHA256:
        raise DataQualityError("G12 frozen group config hash changed")
    ...
    if not isinstance(costs, dict) or not isinstance(long_short, dict):
        raise DataQualityError("G12 frozen cost tables are missing")
    if (
        ...
        raise DataQualityError("G12 frozen cost or borrow-fee contract changed")
...
def _validate_reference_anchor(reference_g00: _G00Reference) -> None:
    if (
        ...
        raise DataQualityError("G12 G00 reference is not the frozen v3-v1 bundle")


def _validate_runtime_roots(config: G12RunConfig) -> None:
    ...
    if (
        ...
        raise DataQualityError("G12 requires sibling data/results local runtime roots")
    try:
        output_root.relative_to(project_root)
    except ValueError:
        pass
    else:
        raise DataQualityError("G12 full bundle cannot be written inside the repository")
    try:
        config.reference_g00_root.resolve().relative_to(output_root)
    except ValueError as error:
        raise DataQualityError(
            "G12 G00 reference must come from the local results root"
        ) from error


def _render_g12_resolved_config_toml(
    *,
    config: G12RunConfig,
    data: LoadedExperimentData,
    ...
    if not rendered.endswith("\n"):
        raise RuntimeError("G12 resolved TOML must end with newline")
    return rendered
...
def _manifest_metadata(
    *,
    ...
    blockers.add("systematic_G12_bundle_is_prototype")
    ...
    return {
        ...
        "risk_rule": {
            ...
            "annual_target_volatility": 0.15,
            "maximum_scale": 1.0,
            "scale_rule": "min(1, 0.15 / current_signal_close_book_RV126)",
        ...
        "runtime_code": {
            "g12_sha256": sha256_file(Path(__file__)),
            "g32_book_helper_sha256": sha256_file(Path(__file__).with_name("g32.py")),
        ...
        "limitations": [
            ...
            "continuous naked-book volatility scaling is a mechanism test, not a deployment claim",
            "G11 and G32 are post-completion report comparators, not runtime references",
        ],
    }
