"""G23 strict-Q4 reversal driven by matching naked-book forecast volatility."""
...
_FROZEN_DESIGN_SHA256 = (
    "1f3c3ea2c9079f6e554acb8643c4875b4ebfbaf03443fdf176578352fbe3a077"
)
_FROZEN_GROUP_CONFIG_SHA256 = (
    "29a79c3e57a15ce73dda3c6ebe5efc39b4b50ec210ea6c0d3ff85456499146c8"
)
...
class G23RunConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.context.group_id != "G23":
            raise ValueError("G23 runner requires the registered G23 spec")
        if isinstance(self.workers, bool) or not isinstance(self.workers, int):
            raise ValueError("G23 workers must be an integer")
        if self.workers <= 0 or self.workers > 8:
            raise ValueError("G23 workers must be between 1 and 8")
...
def run_g23(config: G23RunConfig) -> G23RunResult:
    """Run and freeze the preregistered 72-path G23 mechanism grid."""
    ...
    if config.workers == 1:
        batches = [
            _run_g23_core_batch(context=config.context, data=data, strategies=strategies)
        ]
    else:
...
def _run_g23_core_batch_worker(
    context: ExperimentRunContext,
    ...
    try:
        ...
        raise ValueError(f"unknown G23 worker strategy: {error.args[0]}") from error
    return _run_g23_core_batch(context=context, data=data, strategies=strategies)


def _run_g23_core_batch(
    *,
    context: ExperimentRunContext,
    ...
    *,
    ...
    if len(expected_sessions) != 3_018:
        raise DataQualityError(
            f"G23 requires 3018 naked-book sessions, got {len(expected_sessions)}"
        )
    ...
    output: dict[str, _G23Path] = {}
    for strategy in strategies:
        parent = strategy.parent_id
        if parent is None:
            raise DataQualityError(f"G23 strategy lacks G00 parent: {strategy.strategy_id}")
        if parent not in regime_cache:
            _, engine_start = seed_events[strategy.frequency]
            engine = (
                long_only_engine
                if strategy.portfolio_mode is PortfolioMode.LONG_ONLY
                else long_short_engine
            )
            # Weekly can establish the naked book at t0. Monthly requires the
            # last real pre-t0 execution so that the t0 book return is genuine;
            # all pre-t0 returns remain excluded from the forecast input.
            ...
            if naked.nav.index[0] != engine_start:
                raise DataQualityError(
                    f"G23 naked book did not start on its seed execution for {parent}"
                )
            ...
            if not naked_returns.index.equals(expected_sessions):
                raise DataQualityError(f"G23 naked returns are discontinuous for {parent}")
            regime = strict_lagged_book_forecast_quartiles(
                naked_returns,
                ewma_decay=float(parameters["ewma_decay"]),
                forecast_horizon_sessions=int(
                    parameters["forecast_horizon_sessions"]
                ),
        ...
        if formal[required].isna().any().any():
            raise DataQualityError(
                f"G23 lacks a complete formal state for {strategy.strategy_id}"
            )
        lookback = _variant_lookback(strategy)
        ...
        output[strategy.strategy_id] = _G23Path(
            regime=regime,
...
def _validate_score_switch(
    ...
    if not np.allclose(actual, expected, rtol=0.0, atol=0.0, equal_nan=True):
        raise DataQualityError("G23 score switch differs from the frozen direct rule")
...
def _attach_regime_audit(
    ...
    if sampled[required].isna().any().any():
        raise DataQualityError("G23 book regime is unavailable for a rebalance")
    ...
    if allocation.isna().any() or not np.allclose(
        ...
        raise DataQualityError("G23 direct reversal must retain full requested exposure")
        ...
        output.append(
            {
                ...
                "diagnostic": "causal_book_forecast_state",
                "value": float(row["book_forecast_volatility"])
                if pd.notna(row["book_forecast_volatility"])
                else np.nan,
                "date": pd.Timestamp(date),
                "book_return": float(row["book_return"]),
                "book_forecast_volatility": float(
                    row["book_forecast_volatility"]
                )
                if pd.notna(row["book_forecast_volatility"])
                else np.nan,
                "ewma_variance": float(row["ewma_variance"]),
                "forecast_variance_21": float(row["forecast_variance_21"]),
...
def _frozen_parameters(context: ExperimentRunContext) -> dict[str, object]:
    ...
    if not isinstance(raw, dict) or not isinstance(dates, dict):
        raise ValueError("G23 parameters or program dates are missing")
    ...
    for key, value in expected.items():
        ...
        if not matches:
            raise ValueError(f"G23 frozen parameter changed: {key}={actual!r}")
    history_start = str(dates.get("strategy_forecast_history_start", ""))
    if history_start != "2014-06-30":
        raise ValueError(f"G23 frozen history start changed: {history_start!r}")
    if tuple(context.group.reversal_lookbacks) != (5, 20):
        raise ValueError("G23 reversal lookbacks must remain exactly 5 and 20")
    return {
        "ewma_decay": 0.94,
        "forecast_horizon_sessions": 21,
...
def _validate_frozen_inputs(
    ...
    if context.run_id != "g23-frozen-v3-v1":
        raise DataQualityError("G23 run must use g23-frozen-v3-v1")
    if context.dataset_version != _FROZEN_DATASET_VERSION:
        raise DataQualityError("G23 requires the frozen v3 dataset version")
    if data.dataset_manifest_sha256 != _FROZEN_DATASET_MANIFEST_SHA256:
        raise DataQualityError("G23 frozen dataset manifest hash changed")
    freeze_record = (
    ...
    if (
        ...
        raise DataQualityError("G23 frozen dataset record hash changed")
    design = (
    ...
    if not design.is_file() or sha256_file(design) != _FROZEN_DESIGN_SHA256:
        raise DataQualityError("G23 preregistered design hash changed")
    ...
    if not program.is_file() or sha256_file(program) != _FROZEN_PROGRAM_SHA256:
        raise DataQualityError("G23 frozen experiment program hash changed")
    if sha256_file(context.group.path) != _FROZEN_GROUP_CONFIG_SHA256:
        raise DataQualityError("G23 frozen group config hash changed")
    ...
    if not isinstance(costs, dict) or not isinstance(long_short, dict):
        raise DataQualityError("G23 frozen cost tables are missing")
    if (
        ...
        raise DataQualityError("G23 frozen cost or borrow-fee contract changed")
...
def _validate_reference_anchor(reference_g00: _G00Reference) -> None:
    if (
        ...
        raise DataQualityError("G23 G00 reference is not the frozen v3-v1 bundle")


def _validate_runtime_roots(config: G23RunConfig) -> None:
    ...
    if (
        ...
        raise DataQualityError("G23 requires sibling data/results local runtime roots")
    try:
        output_root.relative_to(project_root)
    except ValueError:
        pass
    else:
        raise DataQualityError("G23 full bundle cannot be written inside the repository")
    try:
        config.reference_g00_root.resolve().relative_to(output_root)
    except ValueError as error:
        raise DataQualityError(
            "G23 G00 reference must come from the local results root"
        ) from error
...
def _validate_main_counts(
    ...
    if len(context.strategies) != 72 or len(summary) != 576:
        raise RuntimeError(
            "G23 requires 72 core paths and 576 scenarios, got "
    ...
    if summary.duplicated(key).any():
        raise RuntimeError("G23 summary contains duplicate scenario identities")
    if int(summary["is_primary_scenario"].sum()) != 72:
        raise RuntimeError("G23 must contain exactly 72 primary scenarios")
    modes = summary.groupby("portfolio_mode", observed=True).size().to_dict()
    if modes != {"long_only": 144, "long_short": 432}:
        raise RuntimeError(f"G23 scenario mode counts changed: {modes}")
...
def _validate_artifact_counts(
    ...
    if len(data.evaluation_sessions) != 2_134 or len(nav) != 1_229_184:
        raise RuntimeError(
            "G23 requires 2134 sessions and 1229184 NAV rows, got "
    ...
    if len(counts) != 576 or not counts.eq(2_134).all():
        raise RuntimeError("G23 every scenario must contain exactly 2134 NAV rows")
    rebalances = artifacts["rebalances"]
    if len(rebalances) != 19_656:
        raise RuntimeError(
            f"G23 requires 19656 primary rebalances, got {len(rebalances)}"
        )
    daily = artifacts["diagnostics"].loc[
        artifacts["diagnostics"]["scope"].eq("daily_naked_regime")
    ]
    diagnostics = artifacts["diagnostics"]
    if not diagnostics["group_id"].astype(str).eq("G23").all():
        raise DataQualityError("G23 diagnostics contain foreign group provenance")
    if len(daily) != 217_296:
        raise RuntimeError(
            f"G23 requires 217296 daily regime rows, got {len(daily)}"
        )
    daily_counts = daily.groupby("strategy_id", observed=True).size()
    if len(daily_counts) != 72 or not daily_counts.eq(3_018).all():
        raise RuntimeError("G23 every core path must contain 3018 daily states")
    if not nav["nav"].map(np.isfinite).all() or not nav["nav"].gt(0.0).all():
        raise DataQualityError("G23 NAV must be finite and positive")
    pnl_columns = [
        "pnl_total",
        "pnl_long_risk",
        "pnl_short_risk",
    ...
    if missing:
        raise DataQualityError(f"G23 NAV lacks P&L columns: {sorted(missing)}")
    numeric = nav[pnl_columns].apply(pd.to_numeric, errors="coerce")
    if numeric.isna().any().any() or not np.isfinite(numeric.to_numpy()).all():
        raise DataQualityError("G23 P&L attribution contains invalid values")
    if float(numeric["pnl_closure_error"].abs().max()) > 1e-12:
        raise DataQualityError("G23 P&L attribution does not close")
    if float(numeric["pnl_unexplained_bridge"].abs().max()) > 1e-10:
        raise DataQualityError("G23 unexplained P&L bridge exceeds tolerance")
    unsupported = (
        numeric["pnl_action_execution_bridge"].abs().gt(1e-10)
        & numeric["pnl_bridge_has_frozen_evidence"].ne(1.0)
    )
    if unsupported.any():
        raise DataQualityError("G23 material action bridge lacks frozen evidence")
...
def _validate_daily_regimes(daily: pd.DataFrame) -> None:
    for strategy_id, frame in daily.groupby("strategy_id", observed=True):
        ...
        if dates.has_duplicates or not dates.is_monotonic_increasing:
            raise DataQualityError(f"G23 daily dates are invalid for {strategy_id}")
        returns = pd.Series(
            pd.to_numeric(ordered["book_return"], errors="coerce").to_numpy(float),
            index=dates,
        )
        direct = strict_lagged_book_forecast_quartiles(
            returns,
            ewma_decay=0.94,
            forecast_horizon_sessions=21,
            history_sessions=756,
        ...
        for column in (
            ...
            if not np.allclose(
                ...
                raise DataQualityError(
                    f"G23 persisted daily {column} is not reproducible"
                )
        ...
        if not np.array_equal(actual_q.fillna(-1).to_numpy(), expected_q.fillna(-1).to_numpy()):
            raise DataQualityError("G23 persisted daily quartile is not reproducible")


def _parent_strategy_id(strategy_id: str) -> str:
    if not strategy_id.startswith("G23__"):
        raise DataQualityError(f"invalid G23 strategy id: {strategy_id}")
    return strategy_id.replace("G23__", "G00__", 1).rsplit("__", 1)[0]
...
def _validate_g00_selection_identity(
    ...
    for row in non_q4.itertuples(index=False):
        ...
        if key not in baseline_lookup.index:
            raise DataQualityError(f"G23 lacks matching G00 rebalance: {key}")
        ...
        for column in (
            ...
            if not matches:
                raise DataQualityError(
                    f"G23 non-Q4 {column} differs from G00 for {key}"
                )
...
def _validate_reversal_cross_signal_identity(rebalances: pd.DataFrame) -> None:
    ...
    parsed = frame["strategy_id"].astype(str).str.extract(
        r"^G23__(?P<signal>[^_]+(?:_[^_]+){2})__top(?P<top>\d+)__"
        r"(?P<frequency>weekly|monthly)__(?P<mode>long_only|long_short)__"
        r"(?P<variant>rev(?:5|20))$"
    )
    if parsed.isna().any().any():
        raise DataQualityError("G23 strategy ids do not match the registered grid")
    ...
    if complete.empty:
        raise DataQualityError("G23 has no common-Q4 cross-signal selection audit rows")
    selected_counts = complete.groupby(key, observed=True)[
        "requested_selected_sids"
    ].nunique()
    if not selected_counts.eq(1).all():
        raise DataQualityError("G23 Q4 pure-reversal selection depends on momentum signal")
...
def _attach_g00_comparisons(
    ...
    for row in summary.itertuples(index=False):
        ...
        if key not in lookup.index:
            raise DataQualityError(f"G23 lacks matching G00 scenario: {key}")
    ...
    if len(comparison) != 2_880:
        raise RuntimeError(
            f"G23 must record 2880 G00 comparison rows, got {len(comparison)}"
        )
    return comparison
...
def _render_resolved_config_toml(
    *,
    ...
    if not rendered.endswith("\n"):
        raise RuntimeError("G23 resolved TOML must end with newline")
    return rendered
...
def _manifest_metadata(
    *,
    ...
    blockers.add("systematic_G23_bundle_is_prototype")
    ...
    return {
        ...
        "risk_rule": {
            "risk_source": "matching G00 naked-book causal EWMA forecast",
            "naked_book_history_start": "2014-06-30",
            "naked_book_cost_bps": 0.0,
            "naked_book_borrow_fee_annual": 0.0,
            "naked_book_cash_return": 0.0,
            "forecast_model": "EWMA adjust=False",
            "ewma_decay": 0.94,
            "forecast_horizon_sessions": 21,
            "annualization": "sqrt(252 * EWMA variance)",
            "quartile_history_sessions": 756,
            "quartile_current_observation_excluded": True,
            "q4_rule": "current forecast volatility strictly greater than lagged q75",
